...
Full reference in `docs/challenge/API.md` (endpoints, filters, pagination). Essentials:
...
Runtime models come from **Regolo.ai** or **Mistral** (free tier, OpenAI-compatible - see `docs/challenge/BRIEF.md`). Use the OpenAI SDK with a custom base URL:
...
All read from `backend/.env` (copy `backend/.env.example`). On Railway, set them with `railway variables --set` (see `docs/challenge/DEPLOY.md`).
...
- Fallback Docker dev environment (`docs/challenge/DOCKER.md`)
...
3. **Hour 2-3**: **first Railway deploy** (see `docs/challenge/DEPLOY.md`), even if rough. Run the platform endpoint check. Surface infra issues with hours of buffer.
